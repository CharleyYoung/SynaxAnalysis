...
//            fout.flush();
        ...
        try {
            switch (mod) {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    inFile = new File("resource/test4.txt");
                    break;
                case 5:
                    inFile = new File("resource/test5.txt");
                    break;
                default:
                    System.out.println("Please enter the correct parameter type");
                    break;
            }
            File outFile = new File("resource/tokenOut.txt");
            if(!outFile.exists()){
    ...
    private int judgeIdentifier(int index,char[] elements){
        try{
            ...
            if(keyWord.contains(res)){
                fout.println(res);
                fout.flush();
            }else {
                fout.print("identifier" + CHANGE_LINE);
                fout.flush();
            }
            return index;
    ...
    private int judgeDecimal(int index,char[] elements){
        ...
        fout.print("decimal"+CHANGE_LINE);
        fout.flush();
        return index;
    }
}
